$(function() {
	...
	function afficher_dates() {
		trouveElementDate();
		var date_now = Math.floor((new Date()).getTime() / 1000) + Math.floor(Cookies.get('dateoffset'));
		...
		for (var j = 0; j < datesTrouvees.length; j++) {
			...
			for (var i = 0; i < intervalleFormattageDates.length; i++) {
				...
				if (Math.abs(age) < intervalleCourant[0]) {
	...
	if (Cookies.get('dateoffset') == null) {
		...
		$.ajax({
			...
			complete: function(data) {
				...
				if (date_sql > 1200000000) // sanity check sur la reponse
					Cookies.set('dateoffset', date_sql - date_now, {
					path: "/"
				});
			}
		});
